...
typedef struct instance_prototype_data INSTANCE_PROTOTYPE_DATA;
...
typedef struct clan_planet_data CLAN_PLANET_DATA;
typedef struct senate_data SENATE_DATA;
typedef struct ship_data SHIP_DATA;
typedef struct missile_data MISSILE_DATA;
typedef struct shockwave_data SHOCKWAVE_DATA;
...
#define PULSE_MOVE_SHIPS          ( 1 ) //insane?
...
typedef enum {
	EVENT_NONE, EVENT_ON_NPC_RAW_KILL
} luaEventTypes;
...
typedef enum
{
   ...
   SUB_WRITING_MAP, SUB_PERSONAL_BIO, SUB_LUA_IMMEDIATE, SUB_REPEATCMD, SUB_RESTRICTED,
   /*
...
#define RACE_ARKANIAN          16
#define RACE_ARKANIAN_OFFSHOOT 17
...
#define VALID_LANGS    ( LANG_COMMON | LANG_WOOKIEE | LANG_TWI_LEK | LANG_RODIAN  \
   ...
   | LANG_TRANDOSHAN | LANG_CHADRA_FAN | LANG_QUARREN | LANG_DUINUOGWUIN | LANG_ARKANIAN)
/*  27 Languages */
...
struct star_data
{
	STAR_DATA *next;
	STAR_DATA *prev;
	STAR_DATA *next_in_starsystem;
	STAR_DATA *prev_in_starsystem;
...
struct dock_data
{
	DOCK_DATA *next;
	DOCK_DATA *prev;
	DOCK_DATA *next_in_planet;
	DOCK_DATA *prev_in_planet;
	PLANET_DATA *planet;
	int vnum;
	const char *name;
};
...
struct planet_data
{
   ...
   CLAN_PLANET_DATA *first_clan_planet_data;
   CLAN_PLANET_DATA *last_clan_planet_data;
...
struct clan_planet_data {
  PLANET_DATA *planet;
  CLAN_DATA *clan;
  CLAN_PLANET_DATA *next; ///soo many LL threads -- my brain!
  CLAN_PLANET_DATA *prev;
  CLAN_PLANET_DATA *prev_in_clan;
  CLAN_PLANET_DATA *next_in_clan;
  CLAN_PLANET_DATA *prev_in_planet;
  CLAN_PLANET_DATA *next_in_planet;
  float alignment;
};
...
struct ship_data
{
   ...
   int lastroom;
   int navseat;
   CHAR_DATA *navpilotch;
   int pilotseat;
   CHAR_DATA *pilotpilotch;
   int coseat;
   CHAR_DATA *copilotch;
   int gunseat;
   CHAR_DATA *gunpilotch;
   long collision;
   SHIP_DATA *target0;
   SHIP_DATA *target1;
   SHIP_DATA *target2;
...
struct shockwave_data 
{
	SHOCKWAVE_DATA *prev;
	SHOCKWAVE_DATA *next;
	SHOCKWAVE_DATA *prev_in_starsystem;
	SHOCKWAVE_DATA *next_in_starsystem;
	SPACE_DATA *starsystem;
	CHAR_DATA *caused_by;
	const char *source;
	float ex;
...
/* Bits for ship flags */
#define SHIP_TRACKINGRADAR	BV00
#define SHIP_AUTOMANEUVER	BV01 //was the last maneuver an automaneuver?
...
//#define ROOM_VIRTUAL		BV12  decided we dont need this
...
#define PLAYER_VIS_RFLAGS	(ROOM_INDOORS | ROOM_BANK | ROOM_PRIVATE | ROOM_SAFE | ROOM_HOTEL | ROOM_FACTORY)
/*
* Directions.
* Used in #ROOMS.
*/
...
struct char_data
{
   ...
   CHAR_DATA *next_in_instance;
   INSTANCE_DATA *instance;
   CHAR_DATA *prev_in_instance;
   CHAR_DATA *master;
   CHAR_DATA *leader;
   FIGHT_DATA *fighting;
   CHAR_DATA *reply;
...
struct pc_data
{
   ...
   const char *pilotprompt;  /* User config prompts */
...
struct instance_prototype_data
{
	//prototype room ranges
	INSTANCE_PROTOTYPE_DATA *prev;
	INSTANCE_PROTOTYPE_DATA *next;
	int low_proto_vnum;
	int high_proto_vnum;
	const char *name;
	int flags;
...
struct instance_data
{
	...
	INSTANCE_DATA *next;
	INSTANCE_DATA *prev;
	INSTANCE_DATA *next_instance;
	INSTANCE_DATA *prev_instance;
...
#define FLINK(link, first, last, next, prev) \
   do                                          \
{                                           \
   if ( !(first) )                          \
{                                        \
...
extern CLAN_PLANET_DATA *first_clan_planet_data;
extern CLAN_PLANET_DATA *last_clan_planet_data;
...
extern STAR_DATA   *first_star;
extern STAR_DATA   *last_star;
...
extern INSTANCE_PROTOTYPE_DATA *first_instance_prototype;
extern INSTANCE_PROTOTYPE_DATA *last_instance_prototype;
...
extern LUA_DATA *first_lua;
extern LUA_DATA *last_lua;
...
DECLARE_DO_FUN( do_luatest );
DECLARE_DO_FUN( do_luarunfile );
DECLARE_DO_FUN( do_luaimmediate);
...
#define LUA_DIR	"../lua/"   /* Main system files    */
...
PLANET_DATA *create_planet(const char *name);
CLAN_PLANET_DATA *get_clan_planet_data(CLAN_DATA *pClan, PLANET_DATA *pPlanet);
CLAN_PLANET_DATA *create_clan_planet_data(CLAN_DATA *pClan, PLANET_DATA *pPlanet);
...
void save_instance_file( char *filename );
