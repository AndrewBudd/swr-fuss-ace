...
void do_score( CHAR_DATA * ch, const char *argument )
{
...

   {
      ...
      for( ability = 0; ability < MAX_ABILITY; ability++ )
         if( ch->skill_level[ability] > 1 || IS_IMMORTAL(ch)  )
...
void do_pilotprompt( CHAR_DATA * ch, const char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   char buf[MAX_INPUT_LENGTH];
   ...
   if( IS_NPC( ch ) )
   {
      send_to_char( "NPC's can't change their pilot prompt..\r\n", ch );
      return;
   }
   argument = smash_tilde_static( argument );
   one_argument( argument, arg );
   if( !*arg )
   {
      send_to_char( "Set pilot prompt to what? (try help pilotprompt)\r\n", ch );
	   if( IS_NPC( ch ) || !ch->pcdata->pilotprompt || !*ch->pcdata->pilotprompt )
		sprintf( buf, "Default pilot prompt string:\n%s\n\r",default_pilot_prompt(ch));
	else
		sprintf( buf, "Current prompt string: %s\n\r", ch->pcdata->pilotprompt );

      send_to_char( buf, ch );
      return;
   }
   if( ch->pcdata->pilotprompt )
      STRFREE( ch->pcdata->pilotprompt );
    ...
    */
   if( !str_cmp( arg, "default" ) )
      ch->pcdata->pilotprompt = STRALLOC( "" );
   else
      ch->pcdata->pilotprompt = STRALLOC( prompt );
   send_to_char( "Ok.\r\n", ch );
   return;
}
