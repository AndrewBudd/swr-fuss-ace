...
#define IDLE_OUT_LD		900 // 15 minutes
#define IDLE_OUT_NOT_PLAYING	900 // 15 minutes
#define IDLE_OUT		12000 // 2 hours
...
void display_pilot_prompt( DESCRIPTOR_DATA * d );
...
/*  time_t	last_check = 0;  */
   ...
   while( !mud_down )
   {
      ...
      for( d = first_descriptor; d; d = d_next )
      {
         ...
         else if( ( !d->character && d->idle/PULSE_PER_SECOND > IDLE_OUT_LD )  /* 15 mins */
                  || ( d->connected != CON_PLAYING && d->idle/PULSE_PER_SECOND > IDLE_OUT_NOT_PLAYING )  /* 15 mins */
                  || d->idle/PULSE_PER_SECOND > IDLE_OUT ) /* 2 hrs  */
         {
   ...
   while( *str != '\0' )
   {
      ...
      else
      {
         switch ( *str )
         {
            ...
            case 'n':
	       if( IS_IMMORTAL( ch) && !can_see(to,ch) )
		  i = "Immortal";
...
char *default_pilot_prompt( CHAR_DATA * ch )
{
   static char buf[MAX_STRING_LENGTH];
   strcpy( buf, "" );
   strcat( buf, "&w{&BTrckng &c[&C%t&c] &BBrng:&C%b&B  &BRnge: &C%r&w}\n" );
   strcat( buf, "&w{&BShld: &C%s&c/%S &BHll: &C%h&c/%H&w} {&BHdng: &C%a&c/%A &BSpd: &C%p&c/%P&w} {&BLsrs: &c[%l&c] &BMssls: &c[%m&c]&w} &W>&w");
   /*strcat( buf, "&BShields:&C%s&B/%S  &BHull:&C%h&B/%H\n" );
   strcat( buf, "&BHeading:&C%a&B  &BTurn: &C%A &BSpeed:&C%p&B/%P\n" );
   strcat( buf, "&BLasers &c[%l&c] Missiles &c[%m&c]\n" );
   strcat( buf, "&BTrack &C[&c%t&C] Bearing:&C%b&B  &BRange: &C%r\n" );
   strcat( buf, "&C >&w" );*/
   return buf;
}
...
void display_pilot_prompt( DESCRIPTOR_DATA * d )
{
   ...
   if( !ch )
   {
      bug( "display_pilot_prompt: NULL ch" );
      return;
   }

   if( !ch->piloting)
   {
      bug( "display_pilot_prompt: NULL ship" );
      return;
   }

   if( IS_NPC( ch ) || !ch->pcdata->pilotprompt || !*ch->pcdata->pilotprompt )
      prompt = default_pilot_prompt( ch );
   else
      prompt = ch->pcdata->pilotprompt;
   ...
   for( ; *prompt; prompt++ )
   {
      ...
      {
         default:
            bug( "Display_pilot_prompt: bad command char '%c'.", *( prompt - 1 ) );
            break;
         case '%':
            *pbuf = '\0';
            pstat = 0x80000000;
            switch ( *prompt )
            {
               case '%':
                  ...
                  *pbuf = '\0';
                  break;
               case 'a':
		  sprintf( pbuf, "%0.2f", radianstodegrees(ch->piloting->heading) );
                  break;
               case 'A':
		  sprintf( pbuf, "%0.2f", radianstodegrees(ch->piloting->turnarc) );
                  break;
               case 'b':
                  if( ch->piloting->track0 != NULL)
			  sprintf( pbuf, "%0.2f", radianstodegrees(s2sbearing(ch->piloting,ch->piloting->track0)) );
		     else
		          strcpy( pbuf, "&zn/a" );
		  break;
               case 'e':
                  pstat = ch->piloting->energy;
                  break;
               case 'E':
                  pstat = ch->piloting->maxenergy;
                  break;
               case 'h':
                  pstat = ch->piloting->hull;
                  break;
               case 'H':
                  pstat = ch->piloting->maxhull;
                  break;
               case 'l':
                  if( ch->piloting->statet0 == LASER_DAMAGED)
                     strcpy( pbuf, "&RDAMAGED" );
                  else if( ch->piloting->statet0 >= ch->piloting->lasers && ch->piloting->lasers != 0 )
                     strcpy( pbuf, "&YRECHARGING" );
                  else if( ch->piloting->statet0 <= ch->piloting->lasers && ch->piloting->lasers != 0)
                     strcpy( pbuf, "&GREADY" );
                  else
                     strcpy( pbuf, "not equipped" );
                  break;
               case 'm':
                  if( ch->piloting->missilestate == MISSILE_DAMAGED)
                     strcpy( pbuf, "&RDAMAGED" );
                  else if( ch->piloting->missilestate != MISSILE_READY )
                     strcpy( pbuf, "&YRELOADING" );
                  else if( ch->piloting->missilestate == MISSILE_READY && ch->piloting->missiles > 0)
                     strcpy( pbuf, "&GREADY" );
                  else if (ch->piloting->missiles <= 0 && ch->piloting->maxmissiles > 0)
                     strcpy( pbuf, "&OEMPTY" );
		  else
		     strcpy( pbuf, "&znot equipped" );
                  break;
               case 'p':
                  pstat = ch->piloting->currspeed;
                  break;
               case 'P':
                  pstat = ch->piloting->realspeed;
                  break;
               case 'r':
                  if( ch->piloting->track0 != NULL)
	                  pstat = ship2shipdist(ch->piloting,ch->piloting->track0);
		     else
		          strcpy( pbuf, "&zn/a" );
		  break;
               case 's':
                  pstat = ch->piloting->shield;
                  break;
               case 'S':
                  pstat = ch->piloting->maxshield;
                  break;
               case 't':
                  if( ch->piloting->track0 != NULL)
	                  strcpy( pbuf, ch->piloting->track0->name );
		     else if (IS_SET(ch->piloting->flightflags,SHIP_TRACKINGRADAR))
		          strcpy( pbuf, "&OSEARCHING" );
		     else
			  strcpy( pbuf, "&zoffline" );
		   break;
            }
            if( pstat != 0x80000000 )
               sprintf( pbuf, "%d", pstat );
