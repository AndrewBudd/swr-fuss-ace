...
void fwrite_char( CHAR_DATA * ch, FILE * fp )
{
   ...
   else
   {
      ...
      if( ch->pcdata->prompt && *ch->pcdata->prompt )
         fprintf( fp, "Prompt       %s~\n", ch->pcdata->prompt );
      if( ch->pcdata->pilotprompt && *ch->pcdata->pilotprompt )
         fprintf( fp, "PilotPrompt       %s~\n", ch->pcdata->pilotprompt );
...
bool load_char_obj( DESCRIPTOR_DATA * d, char *name, bool preload, bool copyover )
{
   ...
   ch->instance = NULL;
   ch->next_in_instance = NULL;
   ch->prev_in_instance = NULL;
   ch->textcolor = 0;
   d->character = ch;
   ch->desc = d;
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'P':
            ...
            KEY( "PilotPrompt", ch->pcdata->pilotprompt, fread_string( fp ) );
         ...
         case 'T':
            ...
            KEY( "Textcolor", ch->textcolor, fread_number( fp ) );
            /*
