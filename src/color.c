...
const short default_set[MAX_COLORS] = {
   ...
   AT_GREY, AT_GREY, AT_GREY, AT_DGREEN, /* 70 */
...
void do_textcolor( CHAR_DATA* ch, const char* argument )
{
	int count;
	int textcolor;
	if( !argument || argument[0] == '\0' )
	{
	   for( count = 1; count < 15; ++count )
	   {
		 ...
		 send_to_pager( "\r\n", ch );
	      ...
	      pager_printf( ch, "%s[%d] %s%-10s", color_str(AT_WHITE,ch), count-1, color_str( count, ch ), pc_displays[count] );
	   }
	   return;
	}
	textcolor = atoi( argument );
	if( textcolor < 0 || textcolor > 14 )
	{
		send_to_pager( "Invalid textcolor, please use the number!", ch);
		return;
	}
	else
	{
		ch->textcolor = textcolor;
		pager_printf( ch,"Your new text color is: [%d] %s%-10s",textcolor,color_str(textcolor+1,ch),pc_displays[textcolor+1]);
	}
