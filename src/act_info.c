      ...
      if( !ch->desc->original )
      {
         if( ( get_trust( ch ) >= LEVEL_IMMORTAL ) )             
         {
            ...
            if( IS_SET( ch->pcdata->flags, PCFLAG_ROOM ) )
            {
                set_char_color( AT_LBLUE, ch );
                send_to_char( "[", ch );
                send_to_char( strupper(flag_string( ch->in_room->room_flags, r_flags )), ch );
                send_to_char( "]", ch );
            }
         }
	 else if((ch->in_room->room_flags & PLAYER_VIS_RFLAGS) != 0)
	 {
		set_char_color( AT_LBLUE, ch);
		send_to_char("[", ch);
		send_to_char(strupper(flag_string(ch->in_room->room_flags & PLAYER_VIS_RFLAGS, r_flags)),ch);
		send_to_char("]", ch);
	 }
      }
...
void do_practice( CHAR_DATA * ch, const char *argument )
{
   ...
   if( argument[0] == '\0' )
   {
      ...
      for( sn = 0; sn < top_sn; sn++ )
      {
         ...
         if( ch->pcdata->learned[sn] == 0 && !IS_IMMORTAL(ch))
            continue;
