...
const char *const r_flags[] = {
   ...
   "nomagic", "bank", "private", "safe", "remove_this_flag", "", "norecall",
...
void do_redit( CHAR_DATA * ch, const char *argument )
{
   ...
   {
      send_to_char( "Deprecated by Miros\r\n", ch );
      return;
      /*argument = one_argument( argument, arg2 );
      if( arg2[0] == '\0' )
      {
...
void edit_buffer( CHAR_DATA * ch, char *argument )
{
   ...
   else
   {
	...
	if( strlen( argument ) > 79 )
      {
	for(wrapped_at = 79; argument[wrapped_at] != ' '; wrapped_at--);
         strncpy( buf, argument, wrapped_at );
         buf[wrapped_at] = 0;
      ...
      else if( strlen( argument ) > 79 )
	{
		if(wrapped_at != 0)
			wrapped_at++;
		//it's a hack...  but in theory....
		edit_buffer(ch,argument+wrapped_at);
	}
...
bool is_valid_vnum( int vnum, short type )
{
   ...
   for( pInstance = first_instance; pInstance; pInstance = pInstance->next )
   {
        if( type == VCHECK_ROOM )
        {
		low_value = pInstance->first_vnum;
		hi_value = pInstance->first_vnum + (pInstance->prototype->high_proto_vnum - pInstance->prototype->low_proto_vnum) -1;
		if( (vnum >= low_value) && (vnum <= hi_value ) )
		{
		    isValid = TRUE;
		    break;
