...
PLANET_DATA *create_planet(const char *name) {
   ...
   if( planet )
   {
	bug("create_planet planet already exists");
	return NULL;
   }
...
CLAN_PLANET_DATA *get_clan_planet_data(CLAN_DATA *pClan, PLANET_DATA *pPlanet) {
	CLAN_PLANET_DATA *pClanPlanetData;
	
	for(pClanPlanetData = first_clan_planet_data; pClanPlanetData; pClanPlanetData = pClanPlanetData->next) {
		if(pClanPlanetData->clan == pClan && pClanPlanetData->planet == pPlanet)
			return pClanPlanetData;
	}
	return NULL;
}
...
CLAN_PLANET_DATA *create_clan_planet_data(CLAN_DATA *pClan, PLANET_DATA *pPlanet) {
	...
	pClanPlanetData = get_clan_planet_data(pClan,pPlanet);
	
	if(!pClanPlanetData) {
		CREATE(pClanPlanetData,CLAN_PLANET_DATA,1);
		LINK(pClanPlanetData,first_clan_planet_data,last_clan_planet_data,next,prev);
		LINK(pClanPlanetData,pClan->first_clan_planet_data,pClan->last_clan_planet_data,prev_in_clan,next_in_clan);
		LINK(pClanPlanetData,pPlanet->first_clan_planet_data,pPlanet->last_clan_planet_data,prev_in_planet,next_in_planet);
		pClanPlanetData->planet = pPlanet;
		pClanPlanetData->clan = pClan;
		pClanPlanetData->alignment = 0.0f;
	}
